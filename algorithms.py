...
# added by SuMPE author: traditional training that takes the class prior of positive and unlabeled data into account (c.f., the xxx_cv version neglects class prior intentionally)
def estimate_preds_standard(df, target, test_data, n_networks=1, lr=1e-4, hid_dim=32, n_hid_layers=1,
                      random_state=None, training_mode='traditional', alpha=None, l2=1e-4, train_nn_options=None,
    ...
    :return: predicted probabilities y(x) of belonging to N rather than P (SuMPE author: size = test_data, be careful, can be mix or pos+mix)
    """

    if train_nn_options is None:
        train_nn_options = dict()

    preds = np.zeros((n_networks, test_data.shape[0],))
    means = np.zeros((n_networks, test_data.shape[0],))
    variances = np.zeros((n_networks, test_data.shape[0],))

    pos_data = df[target == 0] # notice the label convention!
    neg_data = df[target == 1]
    ...
    for i in range(n_networks):      
        ...
        train_NN(neg_data, pos_data, discriminator, d_optimizer,
                    mix_data_test=None, pos_data_test=None, nnre_alpha=alpha,
...
def estimate_preds_cv(df, target, cv=3, n_networks=1, lr=1e-4, hid_dim=32, n_hid_layers=1,
    ...
    :return: predicted probabilities y(x) of belonging to U rather than P (SuMPE author: ignoring relative sizes of U and P, size = pos + mix)
    """
...
def estimate_poster_dedpul(diff, alpha=None, quantile=0.05, alpha_as_mean_poster=False, max_it=100, **kwargs):
    ...
    if alpha_as_mean_poster and (alpha is not None):
        ...
        it = 0
        try_alpha = cur_alpha
        while (abs(cur_alpha - alpha) > kwargs.get('tol', 10**-5)) and (it < max_it): # the em algorithm
...
def estimate_poster_en(preds, target, alpha=None, estimator='e1', quantile=0.05, **kwargs):
    ...
    poster[poster < 0] = 0 # thresholds the value outside region [0, 1]
    return alpha, poster


# ==========================================
# original em, commented out for backup
# ==========================================
def estimate_poster_em(diff=None, preds=None, target=None, mode='dedpul', converge=True, tol=10**-5,
                       ...
                       alpha=None, alpha_as_mean_poster=True, poster_up_bound=10.0, **kwargs):
    """
    ...
    :param poster_up_bound: upper bound of P[Y=1|x], choose 10, 1.0, or < 1.0 (added by SuMPE author)
    ...
    for i in range(max_iterations):
        ...
        # Added by SuMPE author
        # replace using my notation fu = kappa fp + (1-kappa)fn, P[Y=1|x]
        poster_converge_pos = 1 - poster_converge
        kappa_converge = 1 - alpha_converge

        # percent_high_poster_pos = np.sum(poster_converge_pos >= poster_up_bound - 10e-4) / len(poster_converge_pos) # percentage of unreasonably high P[Y=1|x]

        # # when irreducibility is violated, shrink P[Y=1|x] 
        # percent_thresh = 0.05
        # if percent_high_poster_pos > percent_thresh: 
        #     diff = diff / np.mean(diff) # expectation of diff should be 1, an "intentional" error of DEDPUL?
        #     # but is the following necessary???
        #     poster_converge_pos =  0.9 * poster_converge_pos # shrink all
        #     poster_converge = 1 - poster_converge_pos

        mean_poster = np.mean(poster_converge)
        error = mean_poster - alpha_converge

        # if (np.abs(error) < tol) and (percent_high_poster_pos < percent_thresh):
        #     # if poster_up_bound < 1.0 + 1e-4:
        #     print('less than', 100* percent_thresh, '%% of unlabeled data has high P[Y=1|x] >=', poster_up_bound) # for debugging; comment out
        #     break
        # # if np.min(poster_converge) > 0: # should be deleted always true when irreducibility violated
        # #     break
...
# # ========================================
# #          em in different order
# # =========================================

# def estimate_poster_em(diff=None, preds=None, target=None, mode='dedpul', converge=True, tol=10**-5,
#                        max_iterations=1000, nonconverge=True, step=0.001, max_diff=0.05, plot=False, disp=False,
#                        alpha=None, alpha_as_mean_poster=True, poster_up_bound=1.0, **kwargs):
#     """
#     Performs Expectation-Maximization to estimate posteriors and priors alpha (if not provided) of N in U
#         with either of 'en' or 'dedpul' methods; both 'converge' and 'nonconverge' are recommended to be set True for
#         better estimate
...
#         # if percent_high_poster_pos > percent_thresh:
#         #     # poster_converge_pos[poster_converge_pos >= poster_up_bound - 10e-4] *= 0.8
#         #     poster_converge_pos =  0.9 * poster_converge_pos # shrink all
#         #     poster_converge = 1 - poster_converge_pos
#         #     mean_poster = np.mean(poster_converge)
...
#         if (np.abs(error) > tol) or (percent_high_poster_pos > percent_thresh):
#             # print('less than', 100* percent_thresh, '%% of unlabeled data has high P[Y=1|x] >=', poster_up_bound) # for debugging; comment out
#             break
#         # if np.min(poster_converge) > 0: # should be deleted always true when irreducibility violated
#         #     break
