# =================================================
# log of changes:
# This file was originally created by DEDPUL
# SuMPE author changed: estimate_cons_alpha - alpha is the proportion of positive
# ==========================================================
...
def estimate_cons_alpha(dmu, ds, alpha, distribution):
    ...
    (comment by SuMPE author: included general case of gaussian)
    ...
    pm = lambda x: p1(x) * alpha + p2(x) * (1-alpha)

    if dmu == 0 and ds == 1:
        cons_alpha = 0
    elif distribution == 'laplace' and ds == 1:
        cons_alpha = min((pm(-100) / p1(-100)).item(), (pm(100) / p1(100)).item()) # the implementation seems to be wrong when s1 = 2 and s2 = 2
    elif distribution == 'normal' and ds == 1:
        cons_alpha = alpha
    elif dmu == 0:
        cons_alpha = (pm(0) / p1(0)).item()
    else:
        raise NotImplemented
    return cons_alpha

# these two functions are written by SuMPE author
# -----------------------------------------------------------------------------
def estimate_cons_kappa(mu1, s1, mu2, s2, kappa_star, distribution, gamma = 1):
    """
    Estimates kappa^* for limited cases of mixtures of normal and laplace distributions where either dmu=0 or ds=1; 
    (comment by SuMPE author: included general case of gaussian)
    f_p(x) = N(0, 1) or L(0, 1), while f_n(x) = N(mu2, s2) or L(mu2, s2), is specified with parameters

    :param dmu: mu2 - mu1, float
    :param ds: s2 / s1, float
    :param kappa_star: true mixture proportions alpha, float in [0, 1]
    :param distribution: 'normal' or 'laplace'
    :param gamma: in case G is a mixture of distribution G = gamma G1 + (1-gamma) G2
    :return: ground truth proportions alpha^*, float in [0, 1]
    """
    if distribution == 'normal':
        pdf = norm.pdf
    ...
    h = lambda x: pdf(x, mu1, s1)
    g = lambda x: pdf(x, mu2, s2)
    pm = lambda x: (1-kappa_star) * g + kappa_star * h
    ...
    if distribution == 'laplace':
        if ds == 1: 
            cons_kappa = kappa_star + (1-kappa_star) * gamma * np.exp(-np.abs(dmu)/s1)
        else: 
            raise NotImplemented
        ...
        if ds == 1:
            if dmu == 0:
                ...
                cons_kappa = kappa_star
        elif ds < 1: 
            cons_kappa = kappa_star + (1-kappa_star) * gamma * ds * np.exp(- (dmu ** 2)/(2*(s2 ** 2 - s1 ** 2)))
        elif ds > 1: 
            cons_kappa = kappa_star
        else:
            raise NotImplemented
...
def estimate_cons_kappa_empirical(p1, p2):
    """
    :return: kappa(p1|p2) maximum proportion of p2 in p1, and the (region of) location of acchiving it
    """
    x = np.linspace(-20,20,10001)
    ratio_p1_p2 = p1(x) / p2(x)
    kappa = np.min(ratio_p1_p2)
    index = np.where(ratio_p1_p2 <= kappa + 1e-5)
    return kappa, x[[index[0][0], index[0][-1]]]

def estimate_max_poster_empirical(p1, pm, kappa):
    """ 
    : return: maximum posterior probability 
      P[X~p1|x] = kappa * p1/(kappa * p1 + (1-kappa) * p2) = kappa * p1 / pm
    """
    x = np.linspace(-20, 20, 10001)
    post_p1 = kappa * p1(x) / pm(x)
    max_post_p1 = np.max(post_p1)
    index = np.where(post_p1 >= max_post_p1 - 1e-5)
    return max_post_p1, x[[index[0][0], index[0][-1]]]
# -----------------------------------------------------------------------------
