...
# notice: it is not 'standard', this loss eliminates the effect of dataset size of pos and neg, which finally estimates h(x)/[h(x)+f(x)]
def d_loss_standard(batch_mix, batch_pos, discriminator, loss_function=None):
    ...
    return -(torch.mean(loss_function(1 - d_pos)) + torch.mean(loss_function(d_mix))) / 2 # SuMPE author: by taking ``mean'', the shape size of pos and mix are divided


# SuMPE author: this is the `traditional (average) loss` for positive vs. negative training
def d_loss_traditional(batch_mix, batch_pos, discriminator, loss_function=None):
    n_mix = batch_mix.shape[0]
    n_pos = batch_pos.shape[0]
    preds = discriminator(torch.cat([batch_mix, batch_pos]))
    d_mix, d_pos = preds[:n_mix], preds[n_mix:]
    if (loss_function is None) or (loss_function == 'log'):
        loss_function = lambda x: torch.log(x + 10 ** -5)  # log loss
        ...
        loss_function = lambda x: x ** 2  # brier loss
    return -(torch.sum(loss_function(1 - d_pos))  + torch.sum(loss_function(d_mix) )) / (n_mix+n_pos)   # SuMPE author: by taking ``mean'', the shape size of pos and mix are divided
...
def train_NN(mix_data, pos_data, discriminator, d_optimizer, mix_data_test=None, pos_data_test=None,
    ...
    if n_batches is None:
        ...
        batch_size_mix = int(batch_size * mix_data.shape[0]/pos_data.shape[0] ) # SuMPE author made this change 
    else:
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_batches):
            ...
            elif training_mode == 'traditional':
                loss = d_loss_traditional(batch_mix, batch_pos, discriminator, loss_function)
            else:
